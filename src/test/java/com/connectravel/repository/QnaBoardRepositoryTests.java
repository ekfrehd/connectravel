...
public class QnaBoardRepositoryTests {
    ...
    public void testQnaBoardInsert () {
        IntStream.rangeClosed (1, 10).forEach (i -> {//1부터 10까지 생성
            QnaBoard qnaBoard = QnaBoard.builder ().title ("QnA 게시글 더미데이터" + i).content ("QnA 게시글 내용 더미데이터" + i).build ();
    ...
    public void testQnaBoardInsertAddMember () {
        ...
        IntStream.rangeClosed (1, 10).forEach (i -> {//1부터 10까지 생성
            QnaBoard qnaBoard = QnaBoard.builder ().title ("QnA 게시글 더미데이터" + i).content ("QnA 게시글 내용 더미데이터" + i).member (member).build ();
            QnaBoard result = qnaBoardRepository.save (qnaBoard);
            System.out.println ("BNO: " + result.getBno ());
        });
    }
    ...
    public void testQnaBoardReadOne () {
        ...
        qnaBoardRepository.findById (bno);
    ...
    @Test // 게시글 삭제 테스트
    public void testQnaBoardDeleteAddReply () {
        Long bno = 10L;
        //qnaBoardRepository.deleteById (bno);
    }
}
