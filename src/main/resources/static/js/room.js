...
async function loadFetcher() {
    ...
    try {
        ...
        if (!response.ok) {
            let json = await response.json();
            console.error("서버로부터 에러 응답:", json.message);
            return;
        }
        ...
        if (json.result && Array.isArray(json.result)) {
            for (var i = 0; i < json.result.length; i++) {
                let message = json.result[i];

                let str = "<div class='chatbox__messages__user-message'>";
                str += "<div style='float: " + (message.writer === document.getElementById("myName").innerHTML ? "right" : "left") + "' class='chatbox__messages__user-message--ind-message'>";
                str += "<p style='color: #6c757d; size: 3em' >" + message.createdAt + "</p>";
                str += "<p className=\"name\">" + message.writer + "</p>";
                str += "<br/>"
                str += "<span className=\"message\">" + message.message + "</span>";
                str += "</div></div>";
            ...
            console.error("서버로부터의 잘못된 응답 형식.");
        }
    } catch (error) {
