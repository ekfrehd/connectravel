...
public class CrewController {
    ...
    public Response modifyCrew(@PathVariable Long crewId, @RequestBody CrewRequest crewRequest, @AuthenticationPrincipal Member member) {
        return Response.success(crewService.modifyCrew(crewId, crewRequest, member.getUsername()));
    }

    // 크루 게시글 삭제
    @DeleteMapping("/{crewId}")
    @ApiOperation(value = "크루 게시글 삭제", notes = "")
    public Response deleteCrew(@PathVariable Long crewId, @AuthenticationPrincipal Member member) {
        return Response.success(crewService.deleteCrew(crewId, member.getUsername()));
    }

    @PostMapping("/finish")
    @ApiOperation(value = "크루 게시글 모임종료", notes = "")
    public Response finishCrew(@RequestBody CrewRequest crewRequest, @AuthenticationPrincipal Member member) {
        return Response.success(crewService.finishCrew(crewRequest.getId(), member.getUsername()));
    }

    // 크루 게시물 상세 조회
    @GetMapping("/{crewId}")
    @ApiOperation(value = "크루 게시글 상세조회", notes = "")
    public Response detailCrew(@PathVariable Long crewId, @AuthenticationPrincipal Member member) {
