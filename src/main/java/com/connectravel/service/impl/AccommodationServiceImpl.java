...
public class AccommodationServiceImpl implements AccommodationService {
    ...
    public Long registerAccommodation(AccommodationDTO dto) {
        ...
        Member member = memberRepository.findByEmail(dto.getSellerEmail())
                .orElseThrow(() -> new EntityNotFoundException("Member not found with email: " + dto.getSellerEmail()));

        Accommodation accommodation = dtoToEntity(dto);
    ...
    /* 변환 메서드 */
    private Accommodation dtoToEntity(AccommodationDTO dto) {

        Member member = memberRepository.findByEmail(dto.getSellerEmail())
                .orElseThrow(() -> new EntityNotFoundException("이메일이 " + dto.getSellerEmail() + "인 회원을 찾을 수 없습니다."));
