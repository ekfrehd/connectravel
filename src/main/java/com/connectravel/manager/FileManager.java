...
public class FileManager {
    ...
    // 멀티파트 파일과 파일이름을 입력하면 자동으로 파일 생성 (파일이 없을시 저장 안함)
    ...
    // 멀티파트 파일과 파일 이름을 입력하면 자동으로 파일 생성 (리스트 형식의 데이터 png,jpg) 만 저장할 수 있도록 설정할 수 있음
    ...
    // 파일 확장자 검출 기능 filename = 검사할 파일 이름 / CheckList = 체크할 항목(여기 있는 항목의 확장자만 통과 됨)
    public boolean check_extension(String filename, List<String> CheckList) {
        ...
        return CheckList.indexOf(extension) != -1; // -1일시 체크리스트에 없는 확장자이므로 false를 리턴해서 파일 업로드를 제한함.
    }
    ...
    public boolean check_extension(String filename) {
        ...
        return extension != null; // 파일 확장자가 없을 때는 쓰레기 파일이 되므로 기본적으로 검출이 필요함
    }

    // UUID 생성(파일 이름이 겹치지 않게)
