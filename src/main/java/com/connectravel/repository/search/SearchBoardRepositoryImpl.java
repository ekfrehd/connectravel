...
//                .groupBy(qnaBoard);
    ...
    public Page<Object[]> searchPageAdminBaord(String[] type, String category, String keyword, Pageable pageable) {
        ...
        JPQLQuery<Tuple> tuple = from(adminBoard)
                .leftJoin(member).on(adminBoard.member.eq(member))
                .leftJoin(adminReply).on(adminReply.adminBoard.eq(adminBoard))
                .select(adminBoard, member, adminReply.count())
                .where(adminBoard.bno.gt(0L));
            ...
            if (keyword != null) {
                conditionBuilder.or(
                        accommodation.name.contains(keyword)
                                ...
                                .or(accommodation.category.contains(keyword))
    ...
    public Page<Object[]> searchTourBoard(String[] type, String keyword, String category, String region, Pageable pageable) {
        ...
        JPQLQuery<TourBoard> tuple = from(qTourBoard)
                .select(qTourBoard)
                .where(qTourBoard.tbno.gt(0L));
        ...
        if (type != null || region != null || category != null || keyword != null) {
            ...
            if (type == null && keyword != null) {
                conditionBuilder.or(
                        qTourBoard.region.contains(keyword)
                                .or(qTourBoard.content.contains(keyword))
                                .or(qTourBoard.address.contains(keyword))
                                .or(qTourBoard.category.contains(keyword))
                                .or(qTourBoard.title.contains(keyword))
                );
            }
            tuple.where(conditionBuilder);
        }
